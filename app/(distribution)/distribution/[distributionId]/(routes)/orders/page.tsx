...
}> = memo(({ searchParams, params }) => {
  const statuses = use(getStatuses());
  const PENDING_ID = statuses.find((s) => s.name === "PENDING")?.id;
  const redirectPath = `/distribution/${params.distributionId}/orders?statusId=${PENDING_ID}`;
  if (!searchParams?.statusId) redirect(redirectPath);
